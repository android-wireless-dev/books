...
public class BluetoothActivity extends Activity {
    ...
    private static final int BT_DISCOVERABLE_DURATION = 300; // max duration, useful for debuggin //120;
    private static final String DEBUG_TAG = "SimpleBluetooth";
    // generated randomly online; search for "web uuid generate"; and then edited slightly, keep in mind it's hex
    ...
    public void doHandleReceivedCommand(String rawCommand) {
        ...
        if (command.equals(SIMPLE_BT_COMMAND_PING)) {
            ...
            try {
                player.setDataSource(this, Uri.parse("android.resource://com.androidbook.simplewireless/"+R.raw.ping));
                player.prepare();
                player.start();
    ...
    // helper threads
    // server thread; listen for connections. both sides do this. always have to have one server and one client
    // if a client connects, we get a connected BluetoothSocket to use
    // if this device conncets to another server, we'll cancel the listening and use that connection, also a BluetoothSocket
